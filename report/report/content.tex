...
\begin{figure}[h]
    ...
    \caption{Error rate results for CMIM.}
    \label{fig:cmim_errors}
\end{figure}

Figure~\ref{fig:cmim_errors} shows very consistent error rates over different values of $k$. This suggests that the CMIM
algorithm was able to select and rank important features in such a way that the data points for a particular class are closer
together. The error rate was lowest for all labels when $k = 23$. This is a good achievement as the same training parameters can
be used to achieve the best performance for all labels and is overall an improvement as opposed to when there was no feature
selection at all. Almost half of the features were removed, either because they were irrelevant or redundant. When repeating the
experiment, the variance of the error rates were extremely low which shows that the algorithm was able to produce values of a
low uncertainty. This can also be seen by the error bars in Figure~\ref{fig:cmim_errors}.
